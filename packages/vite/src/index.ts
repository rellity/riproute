...
import { installedAdapters, resolveAdapterPackage } from './adapter';
import {
	hasNitroPlugin,
...
function corePlugin(userOptions: RiprouteOptions): Plugin {
	...
	let nitroMode = false;
	/** The adapter this build targets; `null` when nitro drives the build. */
	let adapter: RiprouteAdapter | null = null;
	/** Where the adapter came from, for messages. */
	let adapterFrom = '';
	/** Why no adapter resolved, rethrown if the server entry is ever built. */
	...
	return {
		...
		async config(userConfig, env) {
			...
			// Nitro is not an adapter but a build system that takes over, and its
			// plugin being present is the whole signal. Every other target is the
			// adapter package the app installed — the dependency is the choice,
			// so there is nothing to configure.
			nitroMode = hasNitroPlugin(userConfig.plugins);

			if (!nitroMode) {
				// Resolved here so the ssr environment can read the adapter's Vite
				// contribution, but a failure is *deferred*: a client-only build
				// never touches the server entry and has no business demanding a
				// deployment target. The error surfaces when the entry is built.
				try {
					const selection = await resolveAdapterPackage(root);

					adapter = selection.adapter;
					adapterFrom = selection.from;
				} catch (error) {
			...
			return {
				...
				environments: {
					...
					ssr: {
						...
						// A Worker is uploaded as one module with no node_modules to
						// resolve bare imports against, so its adapter asks for
						// everything to be bundled.
						...(adapter?.viteConfig?.noExternal === true
		...
		async configResolved(resolved) {
			...
			// Nitro drives the whole build, so an adapter package alongside it is
			// dead weight at best and a wrong expectation at worst.
			if (nitroPluginPresent) {
				const spare = installedAdapters(resolved.root);

				if (spare.length > 0) {
					resolved.logger.warn(
						`[riproute] nitro() drives the build, so ${spare.join(', ')} ` +
							'is unused. Remove it, or remove nitro() to deploy with it.'
					);
				}
			}
		...
		async load(id) {
			switch (id) {
				...
				case resolvedId(SERVER_ID): {
					...
					if (adapter === null) {
						throw new Error('[riproute] No adapter resolved for the server entry.');
					}

					// The adapter owns its entry; the plugin only supplies what any
					// target needs to know about this particular build.
					return adapter.entry({
						tags: clientTags,
						template: await fs.readFile(options.template, 'utf-8').catch(() => null),
						templatePath: options.template,
						clientDirFromServer: options.clientDirFromServer,
